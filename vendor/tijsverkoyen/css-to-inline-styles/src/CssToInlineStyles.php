...
class CssToInlineStyles
{
     ...
     * @return string
     */
    public function convert($html,$inline, $css = null)
    {
        ...
        return $this->getHtmlFromDocument($document,$inline);
    }
     ...
     * @return string
     */
    protected function getHtmlFromDocument(\DOMDocument $document,$inline)
    {
        // retrieve the document element
        // we do it this way to preserve the utf-8 encoding
        $htmlElement = $document->documentElement;
        $elements = $htmlElement->getElementsByTagName('style');
         for ($i = $elements->length; --$i >= 0; ) {
            $style = $elements->item($i);
            $style->parentNode->removeChild($style);
        }

        if($inline) {
            $body=$document->getElementsByTagName('body');
            $html = $document->saveHTML($body->item(0)->childNodes->item(0));
            $doctype='';
        }
        else {
